      ...
      <div className="quote-layout">
        ...
        <aside className="quote-summary">
          ...
          <section className="quote-summary-controls" aria-labelledby="quote-calculation-settings">
            ...
            <label><div><strong>세율</strong><small>부가세 적용 비율</small></div><div className="quote-tax-control"><input aria-label="세율" type="number" min="0" max="100" step="1" readOnly={!canWrite} value={Math.round(taxRate * 100)} onChange={(event) => setTaxRate(Number(event.target.value) / 100)} /><span>%</span></div></label>
            <label><div><strong>유효 기간</strong><small>고객이 검토할 수 있는 기한</small></div><input aria-label="견적 유효 기간" type="date" readOnly={!canWrite} value={validUntil} onChange={(event) => setValidUntil(event.target.value)} /></label>
          </section>
      ...
      <form className="outcome-form" aria-busy={busy} onSubmit={async (event) => {
        ...
        <section className="outcome-basics" aria-labelledby="outcome-basics-title">
          ...
          <div className="outcome-metrics-grid">
            <label className="outcome-control outcome-quotation-control"><span>기준 견적</span><div className="outcome-select-shell"><select name="approvedQuotationId" disabled={!canWrite} defaultValue={outcome?.approvedQuotationId ?? ""}><option value="">견적을 연결하지 않음</option>{quotations.filter((quotation) => quotation.status === "PUBLISHED").map((quotation) => <option key={quotation.id} value={quotation.id}>{quotationScenarioLabels[quotation.scenario]} v{quotation.versionNumber} · {formatMoney(quotation.total, quotation.currency)}</option>)}</select><CaretDown size={15} /></div><small>고객이 승인한 견적</small></label>
            <label className="outcome-control"><span>최종 계약 금액</span><div className="outcome-input-shell"><input name="totalRevenue" type="number" min="0" step="0.01" required readOnly={!canWrite} defaultValue={outcome?.totalRevenue ?? ""} /><small>{project.currency}</small></div></label>
            <label className="outcome-control"><span>실제 비용</span><div className="outcome-input-shell"><input name="actualCost" type="number" min="0" step="0.01" required readOnly={!canWrite} defaultValue={outcome?.actualCost ?? ""} /><small>{project.currency}</small></div></label>
            <label className="outcome-control"><span>실제 공수</span><div className="outcome-input-shell"><input name="actualHours" type="number" min="0" step="0.5" required readOnly={!canWrite} defaultValue={outcome?.actualHours ?? ""} /><small>시간</small></div></label>
            <label className="outcome-control"><span>완료일</span><div className="outcome-date-shell"><input name="completedOn" type="date" readOnly={!canWrite} defaultValue={outcome?.completedOn ?? ""} /></div></label>
          </div>
        </section>
        <section className="actual-work-items"><header className="actual-work-heading"><div><span>항목별 실제 결과</span><small>세부 작업을 나누면 다음 견적에서 공수 오차를 비교할 수 있습니다.</small></div>{canWrite && workItems.length > 0 && <button type="button" className="secondary-button" onClick={() => setWorkItems((current) => [...current, { title: "", actualHours: 0, actualCost: 0, notes: "" }])}><Plus size={16} /> 항목 추가</button>}</header>{workItems.length === 0 ? <div className="actual-work-empty"><span className="actual-work-empty-icon"><Graph size={22} /></span><div><strong>세부 항목은 선택 사항입니다.</strong><span>전체 비용과 공수만 기록해도 결과를 저장할 수 있습니다.</span></div>{canWrite && <button type="button" className="secondary-button" onClick={() => setWorkItems([{ title: "", actualHours: 0, actualCost: 0, notes: "" }])}><Plus size={16} /> 첫 항목 추가</button>}</div> : workItems.map((item, index) => <fieldset key={index} disabled={!canWrite}><legend>실제 작업 {index + 1}</legend><div className="form-row"><label>작업명<input required maxLength={200} value={item.title} onChange={(event) => setWorkItems((current) => current.map((currentItem, itemIndex) => itemIndex === index ? { ...currentItem, title: event.target.value } : currentItem))} /></label><label>실제 공수<input type="number" min="0" step="0.5" value={item.actualHours} onChange={(event) => setWorkItems((current) => current.map((currentItem, itemIndex) => itemIndex === index ? { ...currentItem, actualHours: Number(event.target.value) } : currentItem))} /></label><label>실제 비용<input type="number" min="0" step="0.01" value={item.actualCost} onChange={(event) => setWorkItems((current) => current.map((currentItem, itemIndex) => itemIndex === index ? { ...currentItem, actualCost: Number(event.target.value) } : currentItem))} /></label></div><label>작업 메모<textarea rows={3} maxLength={3000} value={item.notes} onChange={(event) => setWorkItems((current) => current.map((currentItem, itemIndex) => itemIndex === index ? { ...currentItem, notes: event.target.value } : currentItem))} /></label>{canWrite && <button type="button" className="remove-feature" onClick={() => setWorkItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash size={16} /> 항목 제거</button>}</fieldset>)}</section>
        <section className="outcome-change-card" aria-labelledby="outcome-change-title">
          ...
          <div className="outcome-change-prompts" aria-label="작성할 내용 예시"><span><Plus size={13} /> 추가된 범위</span><span><Trash size={13} /> 제외된 작업</span><span><Clock size={13} /> 일정·비용 변화</span></div>
          <label htmlFor="outcome-change-reason">변경 내용</label>
          <div className="outcome-change-input"><textarea id="outcome-change-reason" name="changeReason" rows={6} maxLength={5000} readOnly={!canWrite} defaultValue={outcome?.changeReason ?? ""} placeholder="예: 고객 확인이 늦어져 일정이 3일 늘었고, 관리자 통계 화면이 추가되어 개발 공수가 6시간 증가했습니다." /></div>
