...
RAPTOR HTTP 호출은 일반 API의 10초 제한과 분리된 `AGENT_RAPTOR_BUILD_READ_TIMEOUT_MS`를 사용하며,
기본값 310초는 Agent의 기본 build 제한 300초보다 길다.

문서를 추가하거나 보관할 때도 같은 workspace row lock을 먼저 획득하고, 문서 변경과 active pointer 제거,
기존 snapshot의 `SUPERSEDED` 전환을 한 transaction에서 처리한다. 따라서 publish와 원문 변경이 동시에
발생해도 오래된 tree가 active 상태로 남지 않는다.
...
- 문서 변경에 따른 active snapshot 무효화가 호출 transaction에 참여하는지 자동 검사한다.
