...
> 기준일: 2026-08-15
> 기준 문서: `MAIN_PAGE_DESIGN_BRIEF.md`, `VISUAL_DIRECTION.md`, `DESIGN_IMPLEMENTATION_WORKFLOW.md`, `V2_SPECIFICATION.md` §13
...
- `npm.cmd run lint`
- `next build --webpack` (현재 Windows sandbox의 저장소 밖 사용자 경로 `lstat` 제한을 피한 production build)
...
- 실제 browser에서 업무 단계 미니 화면의 단계 전환, horizontal overflow와 console warning·error 0건 확인
...
- 실제 browser의 분석 흐름 미리보기에서 진행률과 활성 단계 일치, horizontal overflow 0px
- 390×844 browser와 6개 프로젝트 fixture에서 sticky 모바일 탐색, 4개 메뉴 각 86px 균등 배치, 프로젝트 선택 후 URL·제목 동기화, horizontal overflow 0px
- 1280×720 계약 fixture에서 프로젝트 현황 3열×2행·카드 본문 약 14px, 완료 분석 결과의 한국어 이벤트·접힌 실행 정보, AI 견적 초안의 한국어 상태·근거 유형, 설정 입력 48px·약 15px와 horizontal overflow 0px 확인
- 390×844 계약 fixture에서 프로젝트 현황 단일 열·짧은 `현황` 메뉴, 설정 본문 319px 단일 열·가로 목차와 horizontal overflow 0px 확인
