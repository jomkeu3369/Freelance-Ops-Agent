...
서비스는 `agent`에 위치한다. `legacy/v1`에는 V1과 기존 prototype
...
- Python runtime의 정확한 버전은 Agent scaffold와 Docker image에서
  ...
  호환성 검증 후 별도 기록한다.
- V2 Agent dependency를 `legacy/v1` Poetry project에 추가하지 않는다.
- `legacy/v1`의 Poetry 파일은 V1·prototype 기준선으로 취급한다. 명시적인
...
- legacy prototype을 V2 Agent로 옮길 때 dependency와 test 경계를 명시적으로
  재정리해야 한다.

## Rejected alternatives

- V2 Agent도 legacy Poetry project에 포함: V1과 V2 dependency 경계가 섞이고
