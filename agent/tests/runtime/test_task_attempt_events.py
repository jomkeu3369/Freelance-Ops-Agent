...
def make_event(*, event_id: str = "event-1", source_event_id: str = "source-1", attempt_id: str = "attempt-1", sequence: int = 1, data: dict[str, object] | None = None, run_id=None) -> TaskAttemptEventWrite:  # noqa: ANN001, E501
    return TaskAttemptEventWrite(event_id=event_id, run_id=uuid4() if run_id is None else run_id, source="worker", source_event_id=source_event_id, task_id="task-1", attempt_id=attempt_id, attempt_number=1, workspace_id=uuid4(), sequence=sequence, event_type="attempt.queued", occurred_at=datetime.now(UTC), data={} if data is None else data)  # noqa: E501
...
async def test_append_rejects_attempt_sequence_collision() -> None:
    ...
    collision = TaskAttemptEventWrite(event_id="event-2", run_id=first.run_id, source="worker", source_event_id="source-2", task_id=first.task_id, attempt_id=first.attempt_id, attempt_number=1, workspace_id=first.workspace_id, sequence=first.sequence, event_type=first.event_type, occurred_at=first.occurred_at, data={})  # noqa: E501
...
async def test_list_for_run_supports_cursor_and_run_isolation() -> None:
    ...
    await store.append(make_event(event_id="event-2", source_event_id="source-2", attempt_id="attempt-2", run_id=run_id))  # noqa: E501
    await store.append(make_event(event_id="other-event", source_event_id="other-source", attempt_id="other-attempt"))
