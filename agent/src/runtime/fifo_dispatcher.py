...
class ResearchFifoDispatcherPilot:
    ...
    def __init__(self, store: PostgresShadowSchedulerStore, sink: FifoDispatchSink, *, resource_pool: str, claimed_by: str, lease_seconds: int = 60) -> None:  # noqa: E501
        if not resource_pool.strip() or not claimed_by.strip() or not 1 <= lease_seconds <= 300:
