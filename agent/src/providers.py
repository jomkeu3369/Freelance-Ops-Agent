        ...
        async def call() -> Any:
            return await client.responses.create(
                ...
                text={
                    "format": {
                        "type": "json_schema",
                        "name": "department_work_product",
                        "strict": True,
                        "schema": DepartmentWorkProduct.model_json_schema(),
                    }
                },
            )

        response, model_calls = await self._invoke(call, max_attempts)
        payload = DepartmentWorkProduct.model_validate_json(str(response.output_text))
...
class GeminiModelProvider(ResilientProvider):
    ...
    async def generate_structured(self, selection: ModelSelection, prompt: str, *, max_output_tokens: int, max_attempts: int | None = None) -> ModelGeneration:  # noqa: E501
        ...
        config = {
            ...
            "response_json_schema": DepartmentWorkProduct.model_json_schema(),
            "max_output_tokens": max_output_tokens,
        }
        ...
        response, model_calls = await self._invoke(call, max_attempts)
        payload = DepartmentWorkProduct.model_validate_json(str(response.text))
