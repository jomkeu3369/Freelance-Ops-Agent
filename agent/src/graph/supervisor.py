...
from contracts import DepartmentName, DepartmentResult, RequestTier
...
class WorkflowState(TypedDict, total=False):
    ...
    department_results: list[dict[str, object]]
...
def global_orchestrator(state: WorkflowState) -> dict[str, object]:
    ...
    if missing_fields:
        return {
            ...
            "error_code": "MISSING_SUPERVISOR_INPUT",
    ...
    try:
        ...
        return {
            ...
            "active_department": None,
            "status": "INPUT_REQUIRED",
            "error_code": "INVALID_REQUEST_TIER",
            "missing_fields": [],
        }
    max_departments = state["max_departments"]
    if not 1 <= max_departments <= len(DEPARTMENT_ORDER):
        return {
            "selected_departments": [],
            "completed_departments": [],
            "department_results": [],
            "active_department": None,
            "status": "INPUT_REQUIRED",
            "error_code": "INVALID_MAX_DEPARTMENTS",
            "missing_fields": [],
        }

    selected = select_departments(request_tier, max_departments)
    return {
...
def department_node(department: DepartmentName) -> Callable[[WorkflowState], dict[str, object]]:
    def run_department(state: WorkflowState) -> dict[str, object]:
        ...
        result = DepartmentResult(
            department=department, status="COMPLETED", summary=f"{department.value} scaffold completed"
        )
