...
```powershell
uv sync --project experiments/runtime_scheduler --group dev
uv run --project experiments/runtime_scheduler pytest experiments/runtime_scheduler
uv run --project experiments/runtime_scheduler python -m experiments.runtime_scheduler.run_experiment
uv run --project experiments/runtime_scheduler python -m experiments.runtime_scheduler.plot_scheduler_simulation
uv run --project experiments/runtime_scheduler streamlit run experiments/runtime_scheduler/streamlit_scheduler_simulation.py
```

plot script가 만드는 PNG와 학습 model JSON은 재생성 가능한 로컬 산출물이므로 Git에 포함하지 않는다.
